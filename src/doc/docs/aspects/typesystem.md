...
> A type system is a part of a language definition assigning types to the nodes in the models written using the language. The type system language is also used to check certain constraints on nodes and their types.
>
...
!!! warning "Type system rules aren't executing."
    ...
    option `#!java do not apply on the fly` is set to true in a checking rule.
...
??? question "How to a build language extension for checking rules?"
...
!!! question "How do you suppress errors?"
    ...
    Make your demonstration node implement the {{ mps_url("@mps.ISuppressErrors") }} interface. If you don't override any methods of it, it will suppress all constraint, typesystem and cardinality errors and warnings.
