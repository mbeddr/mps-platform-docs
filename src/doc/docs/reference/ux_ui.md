...
Use the predefined colors in MPS or define your own colors using the class  `JBColor` with the constructor `JBColor(light theme color, dark theme color)`. By using this class, it shouldn't be necessary to check if the user is [using the Darcula theme](https://stackoverflow.com/c/itemis-mps/questions/426). The MPS colors already support the light and dark theme because they inherit from `JBColor`. As seen in the screenshot below, the colors can be quite different when using the dark theme (e.g., the white and black colors). Don't use hard-coded hex colors or the class [Color](http://127.0.0.1:63320/node?ref=6354ebe7-c22a-4a0f-ac54-50b52ab9b065%2Fjava%3Ajava.awt%28JDK%2F%29%2F%7EColor).
...
Make sure that the foreground color is readable for the chosen background (see screenshots above). You can use an online [contrast checker tool ](https://webaim.org/resources/contrastchecker/) to check that if the color combination is readable.

Example for colors with a good contrast ratio (foreground: black, the ratio is mentioned after the hex code):

![good color ratio](colors_good_contrast_ratio.png)

If this change is not feasible you can for example switch to the default theme (useful for RCPs):
...
- MPS color definitions: [MPSColors](https://github.com/JetBrains/MPS/blob/master/editor/editor-runtime/source/jetbrains/mps/nodeEditor/MPSColors.java)
