...
- checkbox with more than one state
- You have to define states by an enum-property.
...
- extendable, for example, for images
...
- string properties over various lines
...
- displays content in a tabular format
- can be hard-coded or can consist of complex queries/dynamic content
- add new cells and rows in the editor; or it can be just readonly
- need a little experience (trial-and-error)
- Features
    - table cell query
    - partial table
    - grid query
- [Tabular projections in Jetbrains MPS: let’s start building an accounting system](https://tomassetti.me/tabular-projections-in-jetbrains-mps-lets-start-building-an-accounting-system/)(Strumenta's Blog)
- Use `slisson-tables` instead of `mps-tables` because it's more flexible (and not deprecated).
...
- Displays nodes, which aren't directly available in the current context.
...
- tooltips on hover
- Define a regular editor and a hover editor.
...
- Use them together with other languages such as query lists.
