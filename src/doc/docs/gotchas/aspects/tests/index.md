...
!!! question "What is the TestInfo node used for?"
...
??? question "How to set up a generator smoke test?"
    ...
    > I would like to write a generator smoke test.  Therefor I have some solutions with models (regular one, not `@test` models) which are build from command line and the generators invoked are generation some .c files.
    
    > In addition, I also have  a `@test` model in the **same** solution with some unit tests which checks if the output directories of these models contain any generated files. I **do not** invoke the generation of the models programatically but rely on the ant task which is generating the solution.
...
??? question "Why does my node id change during a node test?"
    ...
    ![tests share non check nodes](tests_share_non_check_nodes.png){width="800px"}
    ...
    * I am sure that this does not apply to baseLang-tests since they don't prepare a temporary model for you.
    ...
    - Another source for duplicates is if the test model **imports itself**, as detailed [here][4]
    - If you need full power on the temporary model, consider writing a baseLanguage test and creating your repository and model by hand. MPS authors created the [jetbrains.mps.smodel.TestModelFactory](https://github.com/JetBrains/MPS/blob/master/core/kernel/tests/jetbrains/mps/smodel/TestModelFactory.java) to do that, e.g. the [ModelListenerTest](https://github.com/JetBrains/MPS/blob/master/core/kernel/tests/jetbrains/mps/smodel/ModelListenerTest.java) uses that TestModelFactory. It is unfortunately not available as stubs, so that you'll need to copy it to your project (YSA contains `com.moraad.test.fakeModel` which did that and contains copies for that purpose)

??? question "Editor tests for context assistants?"
...
??? question "How to name checking-errors with dynamic message?"
    ...
    > ![dynamic error message](dynamic_error_message.png)
    ...
    2. Import the language `jetbrains.mps.lang.test` (CMD+L)
    3. Run the intention `Add Message Annotation` (ALT+Enter)
    4. Type a nice name

    > ![dynamic error message fix](dynamic_error_message_fix.png)
